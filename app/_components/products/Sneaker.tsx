...
export default function Sneaker({ children }: { children: React.ReactNode }) {
  return <div className="grid gap-x-2 gap-y-12 grid-cols-2 md:grid-cols-3 lg:grid-cols-3 w-full">{children}</div>;
}

Sneaker.Card = function Card({ children, slug }) {
  return (
    <Link href={`/sneaker/${slug}`} className="group relative flex flex-col h-full">
      {children}
    </Link>
  );
};

Sneaker.Favorite = function Favorite({ isFavoriteState, handleFavorite, isPending }) {
  return (
    <button
      onClick={(e) => {
        e.preventDefault();
...
Sneaker.Info = function Info({ children }) {
  return <div className="mt-4 grid grid-rows-[auto_auto_auto_auto] gap-y-1 min-h-24">{children}</div>;
};

Sneaker.Title = function Title({ children }) {
  return (
    <h3 className="text-sm md:text-base lg:text-lg font-medium text-gray-900 line-clamp-2 leading-snug">{children}</h3>
  );
};

Sneaker.Category = function Category({ children }) {
  return <p className="text-xs md:text-sm lg:text-base text-gray-500 truncate">{children}</p>;
};

Sneaker.Colors = function Colors({ count }) {
  return <p className="text-xs text-gray-500">{count > 1 ? `${count} cores` : `${count} cor`}</p>;
};
      ...
      {sale ? (
        <span className="flex items-center gap-2">
          <span className="font-bold text-primary-600">
            R$ {(sale.discountPrice ?? price * (1 - sale.discountPercentage / 100)).toFixed(2)}
          </span>

          <span className="line-through font-normal text-gray-500">R$ {price}</span>

          <span className="text-green-700 text-sm font-medium">{sale.discountPercentage}% de desconto</span>
        </span>
      ) : (
