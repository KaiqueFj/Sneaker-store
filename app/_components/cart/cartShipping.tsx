...
import { getShippingByCepAction } from '@/actions/address-action';
...
export default function CartShipping() {
  ...
  async function handleCalculateShipping() {
    ...
    const { options, location } = await getShippingByCepAction(rawCep);
  ...
  return (
    <div className="mt-8 flex flex-col gap-4 bg-white rounded-lg">
      <div>
        <h2 className="text-xl font-semibold text-primary-600 mb-1">Frete</h2>
        <p className="text-sm text-primary-400">Digite seu CEP para calcular o frete</p>
      </div>
        ...
        <Form.Field>
          <Form.InputWrapper className="relative">
            <Form.Input
              ...
              value={formatCep(cep)}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleCepChange(formatCep(e.target.value))}
              maxLength={9}
              className="pr-20"
            />
      ...
      {state.shippingOptions.length > 0 && <ShippingOptions shipping={state.shippingOptions} />}
    </div>
  );
}
